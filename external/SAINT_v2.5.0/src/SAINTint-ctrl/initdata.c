...
void read_interaction_data(FILE *fpinter, DATA *data) {
  ...
  tmp_mean /= ((float) ct);
  for(i=0;i<data->ninter;i++) {
    if(data->miss[i] == 0) data->d[i] -= tmp_mean;
  }
  tmp_var = 0.0;
  for(i=0;i<data->ninter;i++) {
    if(data->miss[i] == 0) {
      tmp_var += pow(data->d[i],2.0);
    }
  }
  tmp_var /= ((float) ct);
  for(i=0;i<data->ninter;i++) {
    if(data->miss[i] == 0) data->d[i] /= sqrt(tmp_var);
  }
}
...
void set_ctrlavg(DATA *data) {
  int i;
  for(i=0;i<data->nprey;i++) data->ctrlavg[i] = GSL_NEGINF;
  for(i=0;i<data->ninter;i++) {
    if(data->ctrl[data->i2IP[i]]) {
      if(data->ctrlavg[data->i2p[i]] < data->d[i] && data->miss[i] == 0) data->ctrlavg[data->i2p[i]] = data->d[i];
    }
  }
...
void get_dmin(DATA *data) {
  ...
  dmin = GSL_POSINF;
  for(i=0;i<data->ninter;i++) {
    if(dmin > data->d[i] && data->miss[i] == 0) dmin = data->d[i];
  }
  ...
  /* control */
  data->dmin__ctrl = GSL_POSINF;
  for(i=0;i<data->nprey;i++) {
    for(j=0;j<data->preyNinter[i];j++) {
      id = data->p2i[i][j];
      tmp = data->d[id];
      if(data->dmin__ctrl > tmp && data->miss[id] == 0 && data->ctrl[data->i2IP[id]]) data->dmin__ctrl = tmp;
    }
  }
  // fprintf(stderr, "%.3f\n", data->dmin__ctrl);
  ...
  for(i=0;i<data->nprey;i++) {
    data->ctrl_obs[i] = 0;
    data->dmin_ctrl[i] = GSL_POSINF;
    for(j=0;j<data->preyNinter[i];j++) {
      id = data->p2i[i][j];
      tmp = data->d[id];
      if(data->dmin_ctrl[i] > tmp && data->miss[id] == 0 && data->ctrl[data->i2IP[id]]) data->dmin_ctrl[i] = tmp;
      if(data->miss[id] == 0 && data->ctrl[data->i2IP[id]]) (data->ctrl_obs[i])++;
    }
    if(data->dmin_ctrl[i] == GSL_POSINF) data->dmin_ctrl[i] = data->dmin__ctrl;
  }

  /* interaction - non-control */
  for(i=0;i<data->ninter;i++) {
    data->dmin_inter[i] = GSL_POSINF;
    uid = data->a2u[i];
    if(data->ctrl[data->i2IP[i]] == 0) {
      for(j=0;j<data->n_u2a[uid];j++) {
        id = data->u2a[uid][j];
        tmp = data->d[id];
        if(data->dmin_inter[i] > tmp && data->miss[id] == 0) data->dmin_inter[i] = tmp;
      }
    }
    if(data->dmin_inter[i] == GSL_POSINF) data->dmin_inter[i] = dmin;
  }
...
void initial_impute(PARAM *param, DATA *data, const gsl_rng *r) {
  ...
  for(i=0;i<data->ninter;i++) { 
    if(data->miss[i]) {
      data->d[i] = gsl_ran_gaussian(r, sqrt(param->missVar)) + param->missMean;
    }
  }
}
*/
