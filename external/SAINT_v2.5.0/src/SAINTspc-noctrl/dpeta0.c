...
void DP_eta0_w(PARAM *param, PRIOR *prior, DATA *data, const gsl_rng *r, int pid) {
  int i,j,id;
  float cur_eta, tmp_lambda, maxl, false_avg, tmp;
  ...
  for(i=0;i<_MAX_COMP_;i++) {
    for(j=0;j<data->preyNinter[pid];j++) {
      ...
      if(data->ctrl[data->i2IP[id]] || param->Z[data->a2u[id]] == 0) {
        ...
        prob[i] += log_poisson_g_prop(data->d[id], exp(tmp_lambda), prior->theta_eta0[i]);
      }
    }
    if( data->preyNinter[pid] < ((int) ((param->freqgroup) * ((float) data->nIP))) ) {
      tmp = ((float) data->nIP) - ((float) data->preyNinter[pid]);
      tmp_lambda = false_avg;
      tmp = tmp * log_poisson_g_prop(0.0, exp(tmp_lambda), prior->theta_eta0[i]);
      prob[i] += tmp; 
    }
  }
...
void DP_eta0_theta(PARAM *param, PRIOR *prior, DATA *data, const gsl_rng *r, int pid, int *inuse) {
  int i, j, id, accept;
  float Delta, mhratio, newval, scale, tmp_lambda, false_avg,tmp;
  ...
  if(inuse[pid] == 0) {
    ...
    if(prior->theta_eta0[pid] + Delta <= 1.0 || prior->theta_eta0[pid] + Delta > 100.0) {
      accept = 0;
    }
    else {
      for(i=0;i<data->nprey;i++) {
        ...
        if(prior->w_eta0[i] == pid) {
          for(j=0;j<data->preyNinter[i];j++) {
            ...
            if(data->ctrl[data->i2IP[id]] || param->Z[data->a2u[id]] == 0) {
              tmp_lambda = param->lambda_false[id];
              mhratio += log_poisson_g_prop(data->d[id], exp(tmp_lambda), prior->theta_eta0[pid]+Delta)
                       - log_poisson_g_prop(data->d[id], exp(tmp_lambda), prior->theta_eta0[pid]);
            }
          }
          if( data->preyNinter[i] < ((int) ((param->freqgroup) * ((float) data->nIP))) ) {
            tmp = ((float) data->nIP) - ((float) data->preyNinter[i]);
            tmp_lambda = false_avg;
            tmp = tmp * ( log_poisson_g_prop(0.0, exp(tmp_lambda), param->eta0[i]+Delta) 
      ...
      mhratio += log(gsl_ran_exponential_pdf(prior->theta_eta0[pid]+Delta-1.0, prior->mean_eta0))
               - log(gsl_ran_exponential_pdf(prior->theta_eta0[pid]-1.0, prior->mean_eta0));
      mhratio += -2.0 * (log(prior->theta_eta0[pid]+ Delta) - log(prior->theta_eta0[pid]));
