      ...
      if(prior->w_alpha_prey[i] == pid) {
        for(j=0;j<data->preyNinter[i];j++) {
          ...
          if(param->Z[data->a2u[id]]) {
            ...
            mhratio += log_poisson_g_prop(GSL_MIN(50.0, data->d[id]), exp(param->lambda_true_tmp[id]), param->eta[i]) 
		   - log_poisson_g_prop(GSL_MIN(50.0, data->d[id]), exp(param->lambda_true[id]), param->eta[i]);
          }
        }
      }
    }
