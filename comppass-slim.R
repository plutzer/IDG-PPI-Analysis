...
comppass <-function(input, stats = NULL, norm.factor = 0.98) {
  ...
  ave.psm <- input %>%
    ...
    summarize(Bait = unique(Bait),
              ...
              Entropy = entropy(Spectral.Count)) %>% # Removable?
    ungroup() %>%
    as.data.frame()
  ...
  if(is.null(stats)) {
    stats <- ave.psm
    norm.value <- NULL # So norm factor doesn't matter?
  } else {
  ...
  scores.l <- lapply(seq(nrow(ave.psm)), function(i) { # This is the long step
